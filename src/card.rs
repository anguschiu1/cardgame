...
mod tests {
    ...
    fn can_compare_spotitcard() {
        let card1 = SpotItCard(HashSet::from([SpotItSymbol::Potato]));
        ...
        let card4: SpotItCard =
            SpotItCard(HashSet::from([SpotItSymbol::Potato, SpotItSymbol::Apple]));
        let card5 = SpotItCard(HashSet::from([SpotItSymbol::Potato, SpotItSymbol::Apple]));
    ...
    fn can_match_exactly_one_suit_on_two_spotitcard() {
        ...
        let card3: SpotItCard =
            SpotItCard(HashSet::from([SpotItSymbol::Potato, SpotItSymbol::Apple]));
        let card4: SpotItCard = SpotItCard(HashSet::from([SpotItSymbol::Yogurt]));
